...
class Settings(BaseSettings):
    ...
    def validate_jira_config(cls, v: str, info) -> str:
        """Validate required Jira config fields are non-empty."""
    ...
    def validate_xray_config(cls, v: str, info) -> str:
        """Validate required Xray config fields are non-empty."""
        if not v or not v.strip():
            raise ValueError(f"{info.field_name} is required but not set")
        return v.strip()
    ...
    def validate_base_url(cls, v: str) -> str:
        """Ensure base URLs start with http:// or https://."""
        v = v.strip()
    ...
    def validate_cors_origins(cls, v: str) -> str:
        """Validate CORS origins; warn if set to '*'."""
        v = v.strip()
        if v == "*":
            logger.warning(
...
def load_settings() -> Settings:
    """Load and validate settings from environment variables; exit on error."""
    try:
        return Settings()
