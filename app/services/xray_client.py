...
# Constants
TOKEN_REFRESH_BUFFER_SECONDS = 60  # Seconds before token expiry to refresh (prevents expiry during use)
DEFAULT_TIMEOUT = 30.0  # Default HTTP request timeout in seconds for Xray API calls
MAX_RETRIES = 3  # Maximum number of retry attempts for failed requests


class XrayClient:
    """Async HTTP client for Xray GraphQL API with OAuth 2.0 token management."""
    def __init__(self, base_url: Optional[str], client_id: Optional[str], client_secret: Optional[str], expires_in: int = 86400):
        """Initialize Xray client with OAuth 2.0 credentials."""
    ...
    async def _get_http_client(self) -> httpx.AsyncClient:
        """Get or create a reused HTTP client with connection pooling."""
    ...
    async def close(self):
        """Close HTTP client, release connections and cleanup resources."""
        if self._http_client and not self._http_client.is_closed:
            await self._http_client.aclose()
            logger.info("XrayClient HTTP client closed")

    async def _fetch_token(self):
        """Fetch OAuth 2.0 token using client credentials flow."""
    ...
    async def _ensure_valid_token(self):
        """Ensure valid token exists; refresh if needed (thread-safe with double-check locking)."""
    ...
    async def get_test_case(self, test_case_key: str) -> dict:
        """Fetch Xray test case by key via GraphQL API.

        Args:
            test_case_key: Xray test case key (e.g., 'XSP-123')

        Returns:
            Dict with test case data: key, summary, description, status, priority, testType

        Raises:
            ValueError: If test_case_key is invalid or test case doesn't exist
    ...
    def _is_valid_issue_key(issue_key: str) -> bool:
        """Check if issue key matches Xray format (e.g., PROJ-123)."""
        if not issue_key:
            return False
