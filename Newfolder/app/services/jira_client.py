...
# Constants
DEFAULT_TIMEOUT = 30.0  # Default HTTP request timeout in seconds for Jira API calls


class JiraClient:
    """Async HTTP client for Jira REST API with automatic field resolution."""
    ...
    async def get_issue(self, issue_key: str, include_all_fields: bool = True) -> dict:
        """Fetch Jira issue by key with field name resolution.

        Args:
            issue_key: Jira issue key (e.g., 'PROJ-123')
            include_all_fields: If True, include all custom fields

        Returns:
            Dict with issue data: key, summary, status, priority, assignee, etc. If include_all_fields=True, includes 'all_fields' dict.

        Raises:
            ValueError: If issue_key is invalid or issue doesn't exist
    ...
    def _is_valid_issue_key(issue_key: str) -> bool:
        """Check if issue key matches Jira format (e.g., PROJ-123)."""
        if not issue_key:
            return False
